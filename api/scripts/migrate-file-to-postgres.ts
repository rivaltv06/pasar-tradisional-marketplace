...
import { defaultSettings, newId, nowIso, saveDb, type Db, type UserRow } from '../store.js'
...
async function main() {
  ...
  if (!adminName || !adminEmail || !adminPassword) {
    throw new Error('BOOTSTRAP_ADMIN_NAME/EMAIL/PASSWORD wajib diisi untuk migrasi produksi')
  }
  ...
  const stalls = db.stalls?.length
    ? db.stalls.map((s) => ({ ...s, sellerUserId: admin.id }))
    : [
        {
          id: newId('stl'),
